...
{	
	...
	while(ply = Entities.FindByClassname(ply, "player"))
	{
		if(("hasStats" in ply.GetScriptScope()))
		{
			...
			EntFireByHandle(self, "RunScriptCode", "FixSpeed()", 0.01, ply, null);
...
function DamagePlayer(damage)
{
	...
	printl(damage * resistanceModifierAttribute);
...
function EncounterComplete(difficulty = ENCOUNTERS.easy)
{
	...
	while ( ply = Entities.FindByClassname(ply, "player") )
	{
		if(ply.GetTeam() == 3 || ply.GetTeam() == 2)
		{
...
function AwardPlayer(player, difficulty)
{
	...
	if(difficulty == ENCOUNTERS.bladeKeeper)
	{
...
function ShowMapCommandsToAll(timeToKill=19)
{
	...
	local message = "Map Commands: \n\n"+
					"!mystats ->  Show stats | Посмотреть статы\n"+
					"!levelup stat_name ->  Upgrade stat | Поднять стат\n"+
					"!statsinfo ->  Show ststs information | Информация о статах\n"+
					"!myspells ->  Show all spells/abilites | Посмотреть скиллы\n"+
					"!cast spell_name ->  Use the specified spell/ability | Использовать скилл\n"+
					"!fixspeed ->  Reset speed | Сбросить скорость\n"+
					"!resetpoints ->  Reset points | Сбросить очки\n";
